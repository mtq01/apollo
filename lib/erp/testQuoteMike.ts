...
async function runTests() {
  for (const account of accounts) {                         // loop through each account
    for (const product of products) {                       // loop through each product, for this account
      try {
        console.log(`--- ${account.name} (${account.role} - ${account.accountType}) x ${product.sku} ---`);     // label this test case
        ...
        console.log(`--- ${account.name} (${account.role}) x ${product.sku}: FAILED ---`);
