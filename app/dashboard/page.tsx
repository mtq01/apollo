    ...
    <div className="flex flex-col flex-1 items-center justify-center bg-zinc-50 font-sans dark:bg-black">
      <main className="flex flex-1 w-full max-w-3xl flex-col items-center justify-between py-32 px-16 bg-white dark:bg-black sm:items-start">
        <div className="flex flex-col items-center gap-6 text-center sm:items-start sm:text-left">
          ...
          <h2 className="text-xl font-semibold text-green-500">Component Testing: </h2>
          <Spinner />
          <EmptyState message="test message" />

          {/* ErrorMessage expect the full ErrorType object, not just text, so it looks like this: */}

          <h2 className="text-lg font-semibold text-green-500">ErrorTypes Testing:</h2>
          <ErrorMessage error={{ type: "timeout", message: "timeout test: error"}} />
          <ErrorMessage error={{ type: "not found", message: "nout found: test error"}} />
          <ErrorMessage error={{ type: "restricted", message: "restricted: test error"}} />
          <ErrorMessage error={{ type: "invalid input", message: "invalid input: test error"}} />
          {/* 
          ...
          */}          
          <ErrorMessage error={{ type: "unexpected", message: "Test"} as unknown as ErrorType} />



          {/* Activity Log Output Testing */}
          <h2 className="text-lg font-semibold text-green-500">Activity Log Testing:</h2>
