...
- the prompt should be reconsidered cuz it's sensitive on numbers. once u type "i need nothing" it returns empty [], but once u type "i need a nothing" it fired the error.
- figure out try and catch on for loop in route.ts.
- the prompt should be connected to the actual UI.
- the account should be conected to the login page instead of makinf random account.
